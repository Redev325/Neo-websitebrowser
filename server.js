...
function sendIndexWithBrowserEnhancer(req, res, next) {
  ...
  fs.readFile(indexPath, "utf8", (err, html) => {
    ...
    const scripts = '<script src="/browser-freeze-fix.js?v=3"></script><script src="/browser-enhancer.js?v=6"></script><script src="/explore-card-override.js?v=49"></script>';
    const injected = html.includes('/explore-card-override.js')
      ? html.replace(/<script[^>]+explore-card-override\.js[^>]*><\/script>/gi, '<script src="/explore-card-override.js?v=49"></script>')
